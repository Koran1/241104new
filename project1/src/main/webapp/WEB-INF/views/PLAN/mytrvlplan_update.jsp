...
<title>MyTravelList - 여행계획변경</title>
...
<body>	
    ...
    <div id="container">
		...
		<div id="main_container">
			<form action="mytrvlplan_update_ok" method="post" id="main_form">
				<input type="hidden" name="trvlPlanIdx" value="${trvlPlanIdx}">
				<div id="top-bar">
					<input type="date" name="trvlPlanDate" value="${tplvo.trvlPlanDate}" id="trvlPlanDate">
					<p id="region-filter" data-selected="${tplvo.region}"> ${tplvo.trvlPlanEtc01}</p>
					<input type="text" name="trvlPlanSubject" value="${tplvo.trvlPlanSubject}" id="trvlPlanSubject">
				...
				<!-- Bottom Bar -->
				<div id="bottom-bar">
					<button onclick="updatePlan(this.form)" type="button">수정하기</button>
...
<script
	...
	<script>
        ...
        let addedItems = [];
        document.querySelectorAll(".travelplan").forEach(plan => {
        	const name = plan.getAttribute("data-name");
        	addedItems.push(name);
            const lat = parseFloat(plan.getAttribute('data-lat'));
            const lng = parseFloat(plan.getAttribute('data-lng'));
